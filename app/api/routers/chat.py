...
router = APIRouter(prefix="/api/v1/projects/{project_id}/employees/{employee_id}/chat", tags=["chat"])
...
def send_message(project_id: str, employee_id: str, payload: ChatRequest, repo: JsonRepository = Depends(get_repo)):
    ...
    try:
        response_text = LLMService.generate_chat_response(project, employee, employee.chat_history)
...
def get_chat_history(project_id: str, employee_id: str, repo: JsonRepository = Depends(get_repo)):
...
def clear_chat_history(project_id: str, employee_id: str, repo: JsonRepository = Depends(get_repo)):
    """チャット履歴をクリアします"""
    project = repo.get_project(project_id)
    if not project:
        raise HTTPException(status_code=404, detail=f"Project '{project_id}' not found")
    ...
    repo.save_project(project)
    return {"status": "success", "message": f"Chat history for employee '{employee_id}' has been cleared"}
